...
function hcv = HCV_DiffEq_11152018(t,N,param,nu,pi);
...
% 1x4 matrix of proportion of current PWID that contribute to transmission
inf=zeros(1,4);
for j=1:4;
    inf(1,j) = (N(7,j)+N(8,j)+tau*N(10,j))/(N(6,j)+N(7,j)+N(8,j)+N(9,j)+N(10,j));
end;

% 1x4 matrix of that adjusts proportion of current PWID in each age group 
% that contribute by contact matrix (assortative mixing by age)
con=zeros(1,4);
con=inf*pi;
